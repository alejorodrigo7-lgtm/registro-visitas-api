...
const GestionUsuarios = ({ navigation }) => {
  ...
  // ✅ Función segura para mostrar teléfono
  const mostrarTelefono = (telefono) => {
    if (!telefono) return 'Sin teléfono';
    if (typeof telefono === 'string') return telefono.trim();
    if (Array.isArray(telefono)) {
      const validos = telefono.filter(t => t && typeof t === 'string' && t.trim() !== '');
      return validos.length > 0 ? validos.join(', ') : 'Sin teléfono';
    }
      ...
      <View key={item._id} style={styles.usuarioCard}>
        <View style={styles.usuarioHeader}>
          <View style={styles.usuarioInfo}>
            ...
            {/* ✅ Mostrar teléfono de forma segura */}
            <Text style={styles.usuarioTelefono}>📱 {mostrarTelefono(item.telefono)}</Text>
          </View>
