...
<body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 0; padding: 0; background-color: #f5f0eb;">
  ...
  async enviarNotificacionTicketCreado(ticket) {
    try {
      ...
      console.log(`📧 Enviando notificación de ticket creado: ${ticketId}`);
      ...
      if (emailCliente) {
        ...
        console.log(`⚠️ Cliente sin email: ${nombreCliente}`);
      }

      // 2. Correo para el administrador
      const adminEmail = process.env.ADMIN_EMAIL || 'alejorodrigo7@gmail.com';
  ...
  async enviarNotificacionTicketTomado(ticket, tecnicoNombre) {
    try {
      ...
      console.log(`📧 Enviando notificación de ticket tomado: ${ticket.ticketId}`);
  ...
  async enviarNotificacionTicketEstado(ticket, estadoAnterior, estadoNuevo, observaciones, usuarioNombre) {
    try {
      ...
      console.log(`📧 Enviando notificación de cambio de estado: ${ticket.ticketId} -> ${estadoNuevo}`);
