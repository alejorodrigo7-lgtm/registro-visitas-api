...
const transferenciaSchema = new mongoose.Schema({
  ...
  codigoIdentificador: { type: String, required: true, trim: true },
  nombreUsuario: { type: String, required: true },
  numeroDocumento: { type: String, required: true, trim: true },
  valor: { type: Number, required: true, min: 0 },
  zonaSector: { type: String, enum: ['TOLA', 'SAN JOSE DE CHILIBULO', 'MAGDALENA'], required: true },
  barrio: { type: String, required: true },
  bancoCuenta: { type: String, required: true },
  soporte: { type: String, required: true },
  estado: { type: String, enum: ['SUBIDA', 'CONFIRMADA', 'DENEGADA', 'INGRESADA', 'EN_REVISION'], default: 'SUBIDA' },
