...
const TomarServicio = ({ navigation }) => {
  ...
  const requiereFoto = true;

  // Cargar usuarios (técnicos y jefes)
  useEffect(() => {
    const cargarUsuarios = async () => {
      try {
        ...
        const tecnicosFiltrados = usuariosData.filter(u => u.rol === 'Tecnico');
        const jefesFiltrados = usuariosData.filter(u => u.rol === 'Jefe');
      ...
      <View style={styles.form}>
        ...
        <View style={styles.pickerContainer}>
          <Picker
            ...
            onValueChange={(itemValue) => setFormData(prev => ({ ...prev, tecnicoAsignado: itemValue }))}
            style={styles.picker}
          >
            <Picker.Item label="Selecciona un técnico..." value="" />
            {tecnicos.map((t) => (
              <Picker.Item key={t._id} label={t.nombre} value={t._id} />
            ))}
          </Picker>
        </View>
        ...
        <View style={styles.pickerContainer}>
          <Picker
            ...
            onValueChange={(itemValue) => setFormData(prev => ({ ...prev, jefeAsignado: itemValue }))}
            style={styles.picker}
          >
            <Picker.Item label="Selecciona un jefe..." value="" />
            {jefes.map((j) => (
              <Picker.Item key={j._id} label={j.nombre} value={j._id} />
            ))}
          </Picker>
        </View>
