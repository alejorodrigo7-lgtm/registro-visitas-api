...
const DescargarRecibo = ({ navigation }) => {
  ...
  const fetchSolicitudes = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/solicitudes-recibo?estado=APROBADO`,
        {
          headers: {
  ...
  const handleDownload = async (solicitud) => {
    ...
    Alert.alert(
      '📥 Descargar Recibo',
      `¿Deseas descargar el recibo de ${solicitud.cliente.nombre}?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            Alert.alert('✅ Éxito', 'El recibo se ha descargado correctamente');
          }
        }
      ]
    );
