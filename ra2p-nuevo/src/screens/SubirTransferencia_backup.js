...
const SubirTransferencia = ({ navigation }) => {
  ...
  // ========== NUEVO: Estado para búsqueda de clientes ==========
  const [busquedaCliente, setBusquedaCliente] = useState('');
  const [clientesEncontrados, setClientesEncontrados] = useState([]);
  const [mostrarResultadosBusqueda, setMostrarResultadosBusqueda] = useState(false);
  const [buscandoClientes, setBuscandoClientes] = useState(false);
  
  // ========== NUEVO: Estado para validación de documento ==========
  const [documentoValido, setDocumentoValido] = useState(null); // null: no validado, true: válido, false: inválido
  ...
  // ============================================
  // 🔍 NUEVO: BUSCAR CLIENTES POR CÓDIGO O NOMBRE
  // ============================================
  ...
  const seleccionarCliente = (cliente) => {
    ...
    setClientesEncontrados([]);
    // Resetear validación de documento cuando se selecciona un cliente
    setDocumentoValido(null);
    setMensajeDocumento('');
  };

  // ============================================
  // ✅ NUEVO: VALIDAR NÚMERO DE DOCUMENTO
  // ============================================
  const validarDocumento = async () => {
    ...
    try {
      ...
      const response = await api.get(`/clientes/validar-documento?numero=${encodeURIComponent(documento)}`);
  ...
  const handleSubmit = async () => {
    ...
    // ========== NUEVO: VALIDAR QUE EL DOCUMENTO SEA VÁLIDO ==========
        ...
        <View style={styles.busquedaContainer}>
          <TextInput
            ...
            onChangeText={(text) => {
              ...
              // Si el usuario borra todo, ocultar resultados
              if (text.length === 0) {
                setMostrarResultadosBusqueda(false);
                setClientesEncontrados([]);
              }
        ...
        <View style={styles.documentoContainer}>
          <TextInput
            ...
            onChangeText={(text) => {
              ...
              // Resetear validación cuando cambia el documento
              setDocumentoValido(null);
              setMensajeDocumento('');
            }}
...
const styles = StyleSheet.create({
  ...
  // Estilos de validación de documento
  documentoContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 15,
