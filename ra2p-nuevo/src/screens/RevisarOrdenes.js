...
const RevisarOrdenes = ({ navigation }) => {
  ...
  const cargarRevisar = async () => {
    setLoading(true);
    try {
      const [retiradosRes, revisarRes] = await Promise.all([
        api.get('/recuperacion/ordenes/estado/retirado'),
        api.get('/recuperacion/ordenes/estado/revisar')
      ]);
      
      let data = [];
      if (retiradosRes.data.success) data = data.concat(retiradosRes.data.data);
      if (revisarRes.data.success) data = data.concat(revisarRes.data.data);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.cardInfo}>
        Estado: {item.estado === 'retirado' ? '✅ Retirado' : '📋 Revisar (3 intentos)'}
      </Text>
      <Text style={styles.cardInfo}>👤 Coordinador: {item.coordinadorAsignado?.nombre || 'N/A'}</Text>
      <Text style={styles.cardInfo}>🔄 Visitas: {item.numeroVisitas}</Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <View style={styles.centerContainer}>
          ...
          <Text style={styles.emptyText}>No hay órdenes para revisar</Text>
        </View>
      ) : (
        <FlatList
